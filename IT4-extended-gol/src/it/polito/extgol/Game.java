...
public class Game {
    ...
    private Map<Integer, EventType> events = new HashMap<>();
    ...
    public void setMoods(CellMood mood, List<Coord> coordinates) {
        coordinates.stream().map(c -> board.getTile(c).getCell()).forEach(c -> c.setMood(mood));
    }
    ...
    public static Map<Integer, EventType> loadEvents(Game game) {
        return game.getEventMapInternal();
    }
}
