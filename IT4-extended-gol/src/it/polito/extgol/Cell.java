 ...
 */
@Entity
public class Cell implements Evolvable, Interactable, Comparable<Cell> {
    ...
    @Column(name = "cell_next_mood", nullable = false)
    protected CellMood nextMood = CellMood.NAIVE;

    @Column(name = "death_count", nullable = false)
    protected int deathCount = 0;

    @Column(name = "can_bite_healers", nullable = false)
    protected boolean canBiteHealers = false;
    ...
    public Boolean evolve(int aliveNeighbors) {
        ...
        if (aliveNeighbors > this.type.overpopulationBound) {
            willLive = false;
        }
        // Underpopulation: fewer than 2 neighbors kills a live cell
        else if (aliveNeighbors < this.type.underpopulationBound) {
            willLive = false;
        }
        ...
        if (willLive) {
            this.deathCount = 0;
        } else if (this.deathCount < this.type.survivableDeaths) {
            this.deathCount++;
            willLive = true;
        }
    ...
    public void addLifePoints(int lifepoints) {
        setLifePoints(this.lifepoints + lifepoints);
    }
    ...
    public void interact(Cell other) {
        getMood().interactWith(this, other);
        other.getMood().interactWith(other, this);
    }

    /**
     * Assigns a specific cell type to this cell, influencing its behavior.
     *
     * @param type the CellType to set (e.g., BASIC, HIGHLANDER, LONER, SOCIAL)
     */
    ...
    public void setNextMood(CellMood mood) {
        this.nextMood = mood;
    }
    ...
    public CellMood getNextMood() {
        return this.nextMood;
    }

    public boolean canBiteHealers() {
        return this.canBiteHealers;
    }

    public void canBiteHealers(boolean canBiteHealers) {
        this.canBiteHealers = canBiteHealers;
    }

    @Override
    public int compareTo(Cell o) {
        int y=o.getY()-getY();return y==0?o.getX()-getX():y;/*return Comparator.comparing(Cell::getY)
